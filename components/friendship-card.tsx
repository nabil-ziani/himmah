...
const FriendshipCard = ({ friend, handleAccept, handleReject, handleDelete, isOnline, status }: FriendshipCardProps) => {
    ...
    return (
        <li key={friend.profile.id} className="flex m-5 gap-4">
            <div className={`flex justify-between items-center w-[450px] bg-gray-200 ${!friend.friendship_id && 'border-amber-300 border-2'} rounded-md relative px-2`}>
            ...
            {(status === 'accepted' && friend.friendship_id) && (
