...
const FriendshipCard = ({ friend, handleAccept, handleReject, handleDelete, isOnline, status }: FriendshipCardProps) => {
    ...
    return (
        <li key={friend.id} className="flex m-5 gap-4">
            ...
            {status === 'pending' && (
                <div className="flex gap-2 items-center">
                    <Button size={"sm"} onClick={() => handleAccept!(friend.id)} className="text-white bg-green-500">
                        Accept
                    </Button>
                    <Button size={"sm"} onClick={() => handleReject!(friend.id)} className="text-white bg-[#FF5C5C]">
                        Reject
                    </Button>
                </div>
            )}
